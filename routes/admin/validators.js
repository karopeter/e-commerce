...
module.exports = {
    ...
    requireEmailExists:  check('email').trim().normalizeEmail().isEmail().withMessage('Must provide a vlid email').custom(async (email) => {
        const user = await usersRepo.getOneBy({ email });
        if (!user) {
          throw new Error('Email not found!');
        }
     }),
     requirePasswordForUser: check('password').trim().custom(async (password, { req }) => {
